...
function buildOriginalPath(userId, filename) {
  const ext = path.extname(filename) || '.jpg'
  const base = path.basename(filename, ext)
  const safeBase = base.replace(/[^a-zA-Z0-9-_]/g, '_')
  const unique = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
  const finalName = `${safeBase || 'image'}-${unique}${ext.toLowerCase()}`
  return `${userId}/${finalName}`
}

function buildThumbPath(userId, filename) {
  const base = path.basename(filename, path.extname(filename))
  const safeBase = base.replace(/[^a-zA-Z0-9-_]/g, '_')
  const finalName = `thumb_${safeBase || 'image'}.jpg`
  return `${userId}/${finalName}`
}

export async function uploadOriginal(userId, file) {
  const storagePath = buildOriginalPath(userId, file.originalname || 'image')
...
export async function uploadThumbnail(userId, buffer, filename) {
  const thumbBuffer = await sharp(buffer)
    .resize(300, 300, { fit: 'cover' })
    .jpeg({ quality: 80 })
    .toBuffer()

  const thumbPath = buildThumbPath(userId, filename || 'image')
