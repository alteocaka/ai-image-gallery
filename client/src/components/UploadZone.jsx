...
export default function UploadZone({ onUploaded }) {
  ...
  const addFiles = useCallback((fileList) => {
    ...
    if (invalid.length)
      setError(`Skipped ${invalid.length} file(s): only JPEG and PNG are supported.`);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Uploaded ${uploadedCount} image${uploadedCount === 1 ? '' : 's'}. AI tags will appear after processing.`
      );
      setFiles([]);
  ...
  return (
    <div className="upload-zone">
      ...
      {uploading && !error && (
        <div
          className="upload-zone-progress-wrap"
          role="status"
